...
impl Gcj02 {
    ...
    pub fn to_bd09(self) -> Bd09 {
        ...
        Bd09::new(z * theta.sin() + BD_DLAT, z * theta.cos() + BD_DLON)
    }
}
...
impl Bd09 {
    ...
    pub fn to_gcj02_fast(self) -> Approx<Gcj02> {
        ...
        let z = (x * x + y * y).sqrt() - BD_R_FACTOR * (y * X_PI).sin();
        let theta = y.atan2(x) - BD_THETA_FACTOR * (x * X_PI).cos();
        Approx::new(Gcj02::new(z * theta.sin(), z * theta.cos()), FAST_BOUND_M)
    }
    ...
    pub fn to_gcj02_refined(self) -> Approx<Gcj02> {
        let mut gcj = self.to_gcj02_fast().into_inner();
        ...
        Approx::new(gcj, REFINED_BOUND_M)
    }
}
...
impl Bd09 {
    ...
    pub fn to_wgs84_refined(self) -> Approx<Wgs84> {
        self.to_gcj02_refined()
            .and_then(|gcj| gcj.to_wgs84_refined())
    }
}
...
mod tests {
    ...
    fn wgs84_to_bd09_matches_references() {
        for v in DATUM_VECTORS {
            if let Some(bd) = v.bd09() {
                assert_within_meters(&v.wgs84().to_bd09(), &bd, 0.2);
            }
        }
    }
    ...
    fn gcj02_to_bd09_matches_coordtransform() {
        ...
        let got = Gcj02::new(INPUT.0, INPUT.1).to_bd09();
    ...
    fn bd09_to_gcj02_fast_matches_coordtransform() {
        ...
        let got = Bd09::new(INPUT.0, INPUT.1).to_gcj02_fast();
    ...
    fn gcj02_bd09_refined_round_trip() {
        for v in DATUM_VECTORS {
            let gcj = v.gcj02();
            let back = gcj.to_bd09().to_gcj02_refined();
    ...
    fn bd09_to_wgs84_refined_round_trip() {
        for v in DATUM_VECTORS {
            let back = v.wgs84().to_bd09().to_wgs84_refined();
